...
async function scrapeAllProducts() {
    ...
    fs.writeFileSync(outputPath, JSON.stringify(productsWithBrand, null, 2), 'utf-8');
    ...
    fs.writeFileSync(
        ...
        JSON.stringify(productsWithBrand, null, 2), 
        'utf-8'
    );
    ...
    productsWithBrand.slice(0, 5).forEach((p, i) => {
        console.log(`\n${i + 1}. ${p.name}`);
        console.log(`   Marca: ${p.brand}`);
        console.log(`   Precio: ${p.price}`);
        console.log(`   Imagen: ${p.image ? p.image.substring(0, 50) + '...' : 'No disponible'}`);
    });
